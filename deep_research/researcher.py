...
class DeepResearcher:
    ...
    def _load_text(self, name: str) -> str | None:
        path = self._ckpt(name)
        if not path or not path.exists():
            return None
        return path.read_text(encoding='utf-8')

    def _save_text(self, name: str, content: str) -> None:
        path = self._ckpt(name)
        if not path:
            return
    ...
    async def research(
        ...
        brief = self._load_text('brief.md')
        if brief is None:
            brief = await self.create_brief(query)
            self._save_text('brief.md', brief)
        else:
        ...
        if topics_data is None:
            ...
            status("Loaded research plan from checkpoint")
